...
from modeling_bert import BertForSequenceClassification, BertForSentimentSequenceClassification
...
pretrained_model_name = './bert_base_on_subset_1_train_dev_separeted'
...
testset_path = './dataset/review_subsets/devset_subset_1.json'
"""import data"""
input_ids,attention_masks,labels = get_inputdata(testset_path,tokenizer,maxlength = tokenizer_max_len, dataset = 'review')
print('labels:',labels)
...
for epoch_i in range(0, epochs):
    ...
    for batch in validation_dataloader:
        ...
        for i in range(len(pred_flat)):
            if abs(pred_flat[i] - labels_flat[i]) >= 3:
                # print('true label: ',labels_flat[i])
                # print('pred label: ',pred_flat[i])
                error_text = tokenizer.convert_tokens_to_string(tokenizer.convert_ids_to_tokens(batch[0][i])).split('[SEP]')[0]
                error_examples.append((labels_flat[i],pred_flat[i],error_text))
                # print('=======================================================')
        
    # with open('Error_visulization.txt','w') as errorlog:
    #     for err in error_examples:
    #         errorlog.write(f'True label: {err[0]},\t Pred label: {err[1]}\n')
    #         errorlog.write(err[2])
    #         errorlog.write('\n=======================================================')
    #         errorlog.write('\n======================================================= \n')

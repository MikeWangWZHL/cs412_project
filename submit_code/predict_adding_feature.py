...
from modeling_bert import BertForSentimentSequenceClassification
import json
import torch 
...
from data import get_inputdata,get_input_features
...
pretrained_model_name = './bert_adding_12_features_whole'
...
"""import data"""
input_features_dev = get_input_features(devset_path, dataset = 'review')
...
# input_features_train = get_input_features(trainset_path, dataset = 'review')
...
# train_dataset = TensorDataset(input_ids, attention_masks, labels, input_features_train)
val_dataset = TensorDataset(input_ids_dev, attention_masks_dev, labels_dev, input_features_dev)
...
# pretrain_config['num_features'] = len(input_features_train[0])
# pretrain_config['num_labels'] = 5
# print('using pretrained model: ',pretrained_model_name)
# print('using config: ', pretrain_config)
# print('==================================')
# configuration = BertConfig.from_dict(pretrain_config)
# configuration.update(pretrain_config)

model = BertForSentimentSequenceClassification.from_pretrained(pretrained_model_name)
...
def trim_batch(input_ids, pad_token_id, labels, features, attention_mask=None):
    """Remove columns that are populated exclusively by pad_token_id"""
    keep_column_mask = input_ids.ne(pad_token_id).any(dim=0)
    if attention_mask is None:
        return (input_ids[:, keep_column_mask], None, labels, features)
    else:
        return (input_ids[:, keep_column_mask], attention_mask[:, keep_column_mask], labels, features)
...
for epoch_i in range(0, epochs):
    ...
    #     batch = trim_batch(batch[0],tokenizer.pad_token_id,batch[2],batch[3],attention_mask = batch[1])
    #     if step == 0:
    #         print('after trim')
    ...
    #     outputs = model(b_input_ids, attention_mask=b_input_mask,labels=b_labels,input_features = b_features)
    #     loss = outputs[0] 
